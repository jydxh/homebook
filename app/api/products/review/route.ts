...
import { findExistingReview } from "@/utils/actions/PropertyActions";
...
export const POST = async (request: NextRequest)=>{
  ...
  if(!userId) return NextResponse.json({msg:'unauthorized'},{status:400});
  const body = await  request.json()  as {rating: number | undefined; comment: string | undefined;productId: string | undefined;}
...
export const GET = async(request:NextRequest)=>{
  ...
  if(!productId) return NextResponse.json({msg:'please provide correct params'},{status:400})
